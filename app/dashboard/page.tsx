...
export default function DashboardPage() {
      ...
      <div className="min-h-screen bg-[#0A0A0A] pt-16 flex items-center justify-center">
        <EmptyState
          ...
          action={
            <Link
              href="/auth/login"
              className="text-sm bg-[#6366F1] hover:bg-[#5558E3] text-white px-4 py-2 rounded-md transition-colors"
            >
              Sign in
            </Link>
          }
  ...
  const upcomingBookings = bookings.filter(
    (b) => new Date(b.check_in_date) > now && (b.status === 'pending' || b.status === 'confirmed')
  )
  ...
  const pastBookings = bookings.filter(
    (b) =>
      ...
      b.status === 'no_show' ||
      (new Date(b.check_out_date) < now && (b.status === 'pending' || b.status === 'confirmed'))
  )
        ...
        {bookings.length === 0 ? (
          ...
          <Tabs defaultValue="upcoming" className="w-full">
            <TabsList className="grid w-full grid-cols-4 bg-[#111] border border-[#1F1F1F]">
              <TabsTrigger
                value="upcoming"
                className="data-[state=active]:bg-[#6366F1]/20 data-[state=active]:text-white"
              >
            ...
            <TabsContent value="active" className="space-y-3 mt-6">
              {activeBookings.length === 0 ? (
                <EmptyState
                  ...
                  description="You have no active stays right now"
                />
              ) : (
            ...
            <TabsContent value="past" className="space-y-3 mt-6">
              {pastBookings.length === 0 ? (
                <EmptyState
                  ...
                  description="Your previous stays will appear here"
                />
              ) : (
            ...
            <TabsContent value="cancelled" className="space-y-3 mt-6">
              {cancelledBookings.length === 0 ? (
                <EmptyState
                  ...
                  description="Your cancelled reservations will appear here"
                />
              ) : (
