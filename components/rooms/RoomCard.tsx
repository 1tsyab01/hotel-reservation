...
const roomImages: Record<string, string> = {
  Standard: 'https://images.pexels.com/photos/271624/pexels-photo-271624.jpeg?auto=compress&cs=tinysrgb&w=800',
  Deluxe: 'https://images.pexels.com/photos/164595/pexels-photo-164595.jpeg?auto=compress&cs=tinysrgb&w=800',
  'Junior Suite': 'https://images.pexels.com/photos/1579253/pexels-photo-1579253.jpeg?auto=compress&cs=tinysrgb&w=800',
  'Presidential Suite': 'https://images.pexels.com/photos/262048/pexels-photo-262048.jpeg?auto=compress&cs=tinysrgb&w=800',
}
...
export default function RoomCard({ room, checkIn, checkOut, guests }: RoomCardProps) {
  const imageUrl = roomImages[room.room_types.name] ?? roomImages['Standard']
  const isAvailable = room.status === 'available'

  const href = checkIn && checkOut
    ? `/rooms/${room.id}?checkIn=${checkIn}&checkOut=${checkOut}&guests=${guests ?? 1}`
    : `/rooms/${room.id}`
    ...
    <div className="group surface rounded-xl overflow-hidden hover:border-[#6366F1]/30 transition-all duration-200 hover:shadow-[0_0_30px_rgba(99,102,241,0.08)]">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-2">
          <div>
            <h3 className="text-white font-semibold text-base">Room {room.room_number}</h3>
            <p className="text-white/40 text-xs mt-0.5">Floor {room.floor}</p>
          </div>
        ...
        <div className="flex items-center gap-1 text-white/50 text-xs mb-3">
          ...
          <span>Up to {room.room_types.max_occupancy} guests</span>
        </div>
